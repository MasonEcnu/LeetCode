...
public class RangeSumOfBST938 {
  ...
  public static void main(String[] args) {
    ...
    Objects.requireNonNull(root.getLeft()).setLeft(new TreeNode(3));
    Objects.requireNonNull(root.getLeft()).setRight(new TreeNode(7));

    Objects.requireNonNull(root.getRight()).setRight(new TreeNode(18));
  ...
  private static int rangeSumBSTWithRecursion(TreeNode root, int L, int R) {
    ...
    if (value < L) {
      return rangeSumBSTWithRecursion(root.getRight(), L, R);
    } else if (value > R) {
      return rangeSumBSTWithRecursion(root.getLeft(), L, R);
    } else {
      return value + rangeSumBSTWithRecursion(root.getRight(), L, R) + rangeSumBSTWithRecursion(root.getLeft(), L, R);
    }
  }
  ...
  private static void dfs(TreeNode node, int L, int R) {
    ...
    if (value > L) dfs(node.getLeft(), L, R);
    if (value < R) dfs(node.getRight(), L, R);
